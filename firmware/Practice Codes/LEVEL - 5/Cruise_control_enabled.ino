...
const uint16_t MAX_FRAMES = 1500;     // ~30 seconds at 50 Hz
struct Frame {
  uint8_t a1, a2, a3, a4, a5;   // 0..180
  int16_t mL;                   // (-255..+255)
  int16_t mR;                   // (-255..+255)
};
...
bool cruiseAwaitRelease = false; // avoid toggling off immediately

enum LastAction { ACT_NONE, ACT_LIVE, ACT_X, ACT_Y, ACT_B, ACT_A };
LastAction lastAction = ACT_NONE;

int16_t baseSpeed = 0;         // -255..255
const int SPEED_STEP = 32;     // UP/DOWN bump
const int TURN_MAG   = 200;    // pivot strength during cruise steer
...
void motorsFromDpad(uint8_t dpad, int16_t &mL, int16_t &mR) {
  mL = 0; mR = 0;
  if (dpad & DPAD_UP)        { mL = 255;  mR = 255;  }
  else if (dpad & DPAD_DOWN) { mL = -255; mR = -255; }
  else if (dpad & DPAD_LEFT) { mL = 255; mR = -255;  }
  else if (dpad & DPAD_RIGHT){ mL = -255;  mR = 255; }
}

void computeAndDriveMotorsFromDpad(uint8_t dpad, int16_t &mL, int16_t &mR) {
  motorsFromDpad(dpad, mL, mR);
  bool moving = (mL || mR);
  digitalWrite(motor1_enable_pin, moving ? HIGH : LOW);
  digitalWrite(motor2_enable_pin, moving ? HIGH : LOW);
  controlMotor(motor1_forward_pin, motor1_backward_pin, mL);
  controlMotor(motor2_forward_pin, motor2_backward_pin, mR);
}
...
// -------------------- Your moveBotAndSetServos (unchanged) --------------------
void moveBotAndSetServos() {
  int targetServo1Angle = 170;
  int targetServo2Angle = 60;
  int targetServo3Angle = 10;
  int targetServo4Angle = 120;
...
bool tryExitCruiseOnSelect(ControllerPtr ctl) {
  if (cruiseAwaitRelease) {            // just turned ON; wait release
    if (!ctl->miscSelect()) cruiseAwaitRelease = false;
    return false;
  }
  if (ctl->miscSelect()) {
    while (ctl->miscSelect()) { BP32.update(); delay(5); } // wait release
    cruise = CRUISE_OFF; digitalWrite(led2, LOW);
...
void processGamepad(ControllerPtr ctl) {
  ...
  if (cruise != CRUISE_OFF) {
    ...
    // ------ CRUISE_DRIVE with *opposite-direction* turning ------
    if (cruise == CRUISE_DRIVE) {
      ...
      // LEFT/RIGHT: both sides spin, opposite directions (pivot-like turn)
      if (dpad & DPAD_LEFT) {
        mL +TURN_MAG;  mR = -TURN_MAG;
      ...
      // Servos stay live in cruise
      servo1.write(a1); servo2.write(a2); servo3.write(a3); servo4.write(a4);
      if (ctl->brake())      servo5.write(180);
      else if (ctl->throttle()) servo5.write(0);
